#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""把 invention-patent-skill 装进宿主的技能目录——一条命令，默认不联网。
...
用法:
    ...
    python scripts/install_skill.py --into D:/my/skills/invention-patent-skill   # 精确指定目标目录
    python scripts/install_skill.py --source D:/path/to/checkout    # 从本地另一个副本装
    python scripts/install_skill.py --from-zip invention-patent-skill-vX.Y.Z.zip  # 从 Release ZIP 装（换成实际版本号）
...
安全约定（重要）:
    **默认不覆盖已存在的技能目录。** `--force` 也只在目标目录里确实有 `SKILL.md`
    且其 `name` 就是 `invention-patent-skill` 时才允许删除——这道闸门是为了防
...
#: 任何一处不一致宿主都会找不到技能。
SKILL_NAME = "invention-patent-skill"

#: 上游仓库；`--download` 用它找最新 Release 的 ZIP（公开仓库，不需要 token）。
REPO = "anticipate218/invention-patent-skill"
...
def skills_root(name: str, project_root: pathlib.Path, home: pathlib.Path,
    ...
    返回:
        技能根目录的绝对路径（**不含** `invention-patent-skill` 这一层）。
...
def extract_zip(zip_path: pathlib.Path, workdir: pathlib.Path) -> pathlib.Path:
    ...
    参数:
        zip_path: ZIP 路径（官方 Release 资产，或把本技能目录打成的 ZIP 包都行）。
        workdir: 空的工作目录。
    ...
    陷阱:
        Release ZIP 顶层带 `invention-patent-skill/` 前缀，而手工 `zip -r` 的包可能
...
def _retry_network(what: str, action, *, attempts: int = RETRY_ATTEMPTS,
    ...
    raise InstallError(
        ...
        "手工下载 invention-patent-skill-vX.Y.Z.zip，再用 --from-zip 指过去。"
    )
...
def download_latest_zip(workdir: pathlib.Path) -> pathlib.Path:
    ...
    def fetch_meta():
        req = urllib.request.Request(RELEASE_API, headers={
            ...
            "User-Agent": "invention-patent-skill-installer",
        })
    ...
    def fetch_zip():
        req = urllib.request.Request(str(asset["browser_download_url"]),
                                     headers={"User-Agent": "invention-patent-skill-installer"})
...
def cmd_list_targets(project_root: pathlib.Path, home: pathlib.Path,
    ...
    print("然后用：--into <技能根>            （会自动补一层 invention-patent-skill）")
    print("或：    --into <技能根>/invention-patent-skill")
    return 0
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    print("  2. 让助手读一下这个技能的 SKILL.md（或直接说一句专利相关的需求看它会不会用）。")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="把 invention-patent-skill 装进宿主的技能目录（默认不覆盖、默认不联网）。",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="示例:\n"
               "  python scripts/install_skill.py --list-targets\n"
               "  python scripts/install_skill.py --target auto\n"
               "  python scripts/install_skill.py --into ~/.agents/skills\n"
               "  python scripts/install_skill.py --into ~/.agents/skills/invention-patent-skill\n")
    ...
    parser.add_argument("--into", default="",
                        help="直接指定安装位置（最高优先级）。给技能根会自动补一层 "
                             "invention-patent-skill；给技能目录本身则原样使用")
    parser.add_argument("--source", default="",
                        help="本地技能仓库目录（默认：本脚本所在仓库的根目录）")
    parser.add_argument("--from-zip", default="",
                        help="从本地 ZIP 安装（官方 Release 资产，或本技能目录打成的 ZIP）")
