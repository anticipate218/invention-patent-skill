...
技能不只服务某一个技术领域。机械、电子、通信、软件算法、化学材料、生物医药走的是**同一条流水线**，领域差异只改变每层要填的技术内容和专项审查入口。完整定义见 `references/paradigm.md`。

抽象成一句话：

> 一份发明专利申请 = **一个明确的技术问题** + **为解决它而共同作用的一组技术特征** + **足以让本领域技术人员实现该方案的公开**。

| 层 | 产出 | 通过门槛 |
|---|---|---|
| 0 事实 | 事实清单 / 推测清单 / 未知清单 | 三者已分开，未知项没有被自动补齐 |
| 1 发明点 | "技术问题—区别特征—技术效果"三元组 | 每个效果都能回指到具体特征或特征组合 |
| 2 技术模型 | 对象、动作、关系、参数与术语表 | 同一对象只有一个主名称；关系足以复原方案 |
| 3 权利 | 独权（最小必要特征集）+ 从权退守树 | 独权含必要特征；从权逐层增加限定 |
| 4 支撑 | 说明书五部分与实施例 | 权项每个特征都有说明书依据；方案能够实现 |
| 5 公开 | 摘要、摘要附图、附图说明与标记表 | 术语与标记双向一致 |
| 6 交付 | 自检报告、待确认清单、正式版式 | 机械检查通过且未确认事项显式保留 |

**反向回归**是范式的一部分：权项缺支撑就回到事实层补问，而不是在权项里凭空补特征；摘要出现新内容就删除或先确认原始记载；检索发现独权过宽就重做发明点三元组。

各领域的路由表（机械/电子通信/软件 AI/化学材料/中药/生物序列/工艺方法）见 `references/paradigm.md` §10。
...
│   ├── paradigm.md               # 通用生成范式：领域无关的七层流水线与领域适配矩阵
